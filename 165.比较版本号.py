...
class Solution:
    ...
    #     lenV1 = len(version1)
    #     lenV2 = len(version2)
    #     while p1 < lenV1 or p2 < lenV2:         #同时遍历
    #         num1, p1 = self.nextBlock(version1, lenV1, p1)  #获取两个”.“之间的数据，同时后移指针
    #         num2, p2 = self.nextBlock(version2, lenV2, p2)
    #         if num1 != num2:
    #             return 1 if num1 > num2 else -1
    #     return 0
    
    # def nextBlock(self, version, lenV, p):
    #     if p > lenV - 1:        #超出版本长度，返回0
    #         return 0, p
    #     end = p
    #     while end < lenV and version[end] != '.':   #找到当前block的尾部
    #         end += 1
    #     num = int(version[p:end]) if end != lenV else int(version[p:lenV])  #block内字符串转换成数字
    #     p = end + 1     #指针后移到下个block
    #     return num, p
