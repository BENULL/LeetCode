/*
 * @lc app=leetcode id=690 lang=scala
 *
...
object Solution {
    def getImportance(employees: List[Employee], id: Int): Int = {
        val empLookup = employees.foldLeft(Map[Int,Employee]())((map,e)=> map+(e.id->e))

        def dfs(employeeId:Int,importanceAcc:Int): Int ={
            val e = empLookup(employeeId)
            e.subordinates.foldLeft(e.importance){
                case (importanceAcc,subId) => {
                    val subImportance = empLookup(subId).importance
                    importanceAcc + dfs(subId,subImportance)
                }
                case _ => importanceAcc
            }
        }
        dfs(id,0)
